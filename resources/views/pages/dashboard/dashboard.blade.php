...
@if ($user->verification_status === 'unverified')
    <div class="fade-up mb-4 w-full flex flex-col bg-gradient-to-r from-red-50 to-red-100 border border-red-200 rounded-xl px-6 py-4 shadow-sm gap-4">
        <div>
...
@elseif($nursery === null)
    <div class="fade-up mb-4 w-full flex flex-col bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-xl px-6 py-4 shadow-sm gap-4">
        <div>
            <h2 class="text-gray-900 font-bold text-lg leading-snug">
...
<div class="fade-up bg-gray-200 w-full flex flex-col rounded-xl px-6 py-4">
    ...
    <div class="flex items-center gap-2 w-full mb-2">
        ...
        <div class="relative">
            @if ($user->avatar)
                <img src="{{ $user->avatar }}" class="w-12 h-12 rounded-full object-cover border-2">
            @else
                <div class="w-12 h-12 rounded-full bg-green-800 text-white flex items-center justify-center text-lg font-bold border-2 border-white">
            ...
            {{-- Verification Badge --}}
            @if ($user->verification_status === 'unverified')
                <span class="absolute -bottom-0 -right-0 bg-red-600 rounded-full p-0.5 shadow-md" title="Unverified Account">
            ...
            @elseif($user->verification_status === 'verified')
                <span class="absolute -bottom-0 -right-0 bg-green-600 rounded-full p-0.5 shadow-md" title="Verified Account">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z" />
            ...
            <p class="text-gray-800 font-semibold truncate flex items-center gap-2">
                ...
                @if ($user->subscription_type === 'premium')
                    <img width="20" height="20" class="mb-1 -ml-1"
                        src="https://img.icons8.com/ios-filled/50/FAB005/crown.png" alt="crown" title="Premium Member" />
                @elseif($user->subscription_type === 'general')
                    <span class="bg-gray-400 rounded-md font-bold text-white px-2 text-xs -ml-1" title="General Member">Free</span>
                @endif
            </p>
